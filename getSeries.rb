...
$log = Logger.new($cnf['getSeries']['logfile']) #Logfile
			...
			sResult.css('div.eintrag2 a').each do |sDownLink|
				sHoster.each do |h|
					if sDownLink.content.include?(h)    
						...
						$log.info "Found #{sSearch} with Link #{sDownLink['href']}"
						pushToPebble("getSeries","Downloading #{sSearch}") #Pushed eine Message per PushOver an ein Smartphone / Pebble
					end
				end
			end
		end
...
def getDownLink(sBaseurl, sSearch)
		...
		begin
			...
			if sLink1080p != ''
				...
				sLink = sLink1080p
				$log.info "Getting #{sSearch} in 1080p"
			elsif sLink720p != ''
				$osSerie.squali = '720p'
				sLink = sLink720p
				$log.info "Getting #{sSearch} in 720p"
			elsif sLinkXVID != ''
				# Wenn nur xvid Qualität gefunden wurde, wird nach dem 5, versuch eine bessere Quali zu finden, xvid geladen
				$log.info "Found #{sSearch} only in xvid"
				$log.info "Checking last found Quality for #{sSearch} in xVid"
				if $osSerie.squali == 'xvid'
					$log.info "Last found Quality for #{sSearch} was xvid, checking allready tried attempts"
					$log.info "Tried to find in better quality #{$osSerie.sattempts} times"
					if $osSerie.sattempts.to_i <= 5
						$osSerie.sattempts = ($osSerie.sattempts.to_i + 1)
						$log.info "Rescheduling #{sSearch}"
						sLink = ''
					else
						$log.info "This is the #{$osSerie.sattempts} try downloading #{sSearch} now in xvid"
						$osSerie.sattempts = "0"
						$log.info "Setting Attempts to #{$osSerie.sattempts}"
						sLink = sLinkXVID
					end
				end
			end
					...
					if sLink != ''
						if downloadSeries(sLink, sSearch, sHoster)
							...
							if $osSerie.sepisode != "01"
								...
								sLink = getDownLink(sBaseurl, sSearch)
								if downloadSeries(sLink, sSearch, sHoster)
								  $log.info "Next Season of #{sSeries} has started"
